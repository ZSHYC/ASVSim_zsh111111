...
Accurate instance segmentation of ice features is essential for navigation planning. The Segment Anything Model (SAM), developed by \citeauthor{kirillov2023segment}, provides a foundation model for image segmentation that can generalize to novel objects with minimal prompting. SAM2 \parencite{ravi2024sam2} extended this to video sequences with improved temporal consistency and the ability to track objects across frames.

SAM3 \parencite{kirillov2026sam3} represents the latest advancement in the series, introducing a unified architecture capable of processing both images and video through a single model. With 848 million parameters, SAM3 combines a DETR-based detector with an enhanced SAM2 tracker, supporting three prompting modes: text prompts, visual prompts, and example-based prompts. This unified approach is particularly relevant for polar navigation where temporal consistency in video segmentation is crucial for tracking dynamic ice conditions. The model's zero-shot capability enables segmentation of diverse ice formations without task-specific training, making it suitable for the variable ice conditions encountered in Arctic environments.

Recent work has explored adapting SAM for marine environments. \citeauthor{zhang2024marine} proposed specialized fine-tuning strategies for ship and iceberg detection using SAM-based architectures. The application of SAM3 to sea ice segmentation represents a novel contribution of this thesis, demonstrating the practical utility of foundation models in maritime perception tasks.

\subsubsection{Depth Estimation}

Monocular depth estimation has progressed significantly through the Depth Anything series. Depth Anything V1 \parencite{depthanything2023} established a robust baseline using a large-scale dataset and Vision Transformer (ViT) encoders. Depth Anything V2 \parencite{depthanythingv22024} improved generalization through better data curation and architectural refinements, achieving superior performance on outdoor scenes.

Depth Anything 3 \parencite{zhang2025depthanything3} represents the current state of the art, introducing several architectural innovations over its predecessors. The model employs a simplified plain transformer architecture based on DINOv2, replacing complex multi-task learning heads. Key capabilities include multi-view joint processing for spatially consistent geometry, direct camera pose prediction without known camera parameters, and output of 3D Gaussian Splatting rendering parameters. The model is available in multiple configurations, from 0.08B parameter base models to 1.4B parameter nested variants supporting metric depth and 3D Gaussian output.

Despite impressive benchmark results showing 44.3\% improvement in camera pose accuracy compared to VGGT, empirical evaluation in this thesis revealed that DA3's pose prediction exhibits limitations in simulation environments with coordinate systems requiring precise absolute positioning. This finding highlights the importance of domain-specific validation of foundation model outputs and informs the design of robust perception pipelines that combine model predictions with ground truth data when available.

For metric depth estimation, \citeauthor{ranftl2020towards} demonstrated that combining monocular depth with multi-view constraints can achieve accuracy competitive with active sensors, supporting the approach taken in this work of fusing DA3 depth estimates with simulation ground truth.
