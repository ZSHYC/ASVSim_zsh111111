...
\textbf{2. Rendering Optimization:} Identified and resolved critical performance bottlenecks, achieving a 9$\times$ speed improvement (from 55--60 seconds to 6--7 seconds per frame) through disabling Lumen global illumination and reducing resolution to 640$\times$480. This optimization transforms impractical data collection into an efficient workflow.

\textbf{3. Multi-Modal Data Pipeline:} Developed an automated collection system that captures synchronized RGB images, depth maps, LiDAR point clouds, and accurate ground truth poses. The pipeline includes robust error handling, retry mechanisms, and automatic validation.

\textbf{4. Data Format Integration:} Implemented conversion to COLMAP format, enabling direct use of collected data with standard 3D Gaussian Splatting training pipelines. The format preserves accurate camera intrinsics and extrinsics from simulation.

\textbf{5. Experimental Validation:} Demonstrated the system through successful collection of 200-frame datasets with 100\% success rate, validating both the technical implementation and practical usability.
...
\begin{itemize}
    ...
    \item Implement real-time segmentation using SAM3
...
\begin{itemize}
    ...
    \item Integrate depth estimation (Depth Anything 3) for perception
    \item Implement dynamic obstacle handling
